...
struct ssl_st {
    /*
     * WebKit ABI shim: the shipped webOS libWebKitLuna inlines
     * SSL_get_SSL_CTX(ssl) as a read of *(ssl + 0xD8) (the 0.9.8 offset of
     * ssl->ctx). Relocate ctx to that exact offset so the device browser's
     * WebCore::CurlHandle::sslVerificationCallback gets the real SSL_CTX*.
     * (OpenSSL itself accesses ctx by name, so its offset is irrelevant to us.)
     */
    char _wk_lead[0xD8];
    SSL_CTX *ctx;
    /*
...
# endif
    ...
    /* ctx relocated to offset 0xD8 at the top of this struct (WebKit ABI) */
...
# endif                         /* OPENSSL_NO_EC */
    ...
    /* Palm/webOS TLS extension compatibility fields */
    struct {
        TLS_EXTENSION *hello_ext;
        tls_extension_cb_fn ext_cb;
        void *ext_cb_arg;
    } palm_ext;
