...
void tls1_free(SSL *s)
{
    ...
    /* Clean up Palm/webOS extension data */
    if (s->palm_ext.hello_ext != NULL) {
        OPENSSL_free(s->palm_ext.hello_ext);
        s->palm_ext.hello_ext = NULL;
    }
    ssl3_free(s);
}

/*
 * Palm/webOS TLS extension compatibility layer
 *
 * These functions provide backward compatibility with the Palm/webOS
 * SSL_set_hello_extension() API that was used in OpenSSL 0.9.8.
 */

/* Free Palm extension data - public function */
void SSL_free_hello_extension(SSL *s)
{
    if (s == NULL)
        return;
    ...
    s->palm_ext.ext_cb = NULL;
    s->palm_ext.ext_cb_arg = NULL;
}

/*
 * SSL_set_hello_extension - Set a TLS extension to be sent in ClientHello
 *
 * This is a Palm/webOS compatibility function. It sets extension data that
 * will be included in the ClientHello message.
 *
 * Note: In OpenSSL 1.1.1, custom extensions are normally registered at the
 * SSL_CTX level using SSL_CTX_add_custom_ext(). This function provides
 * backward compatibility by storing the extension data, but applications
 * using this API should also register appropriate extension handlers at
 * the CTX level for the extension to be sent/received.
 *
 * Parameters:
 *   s        - SSL connection
 *   ext_type - Extension type number
 *   ext_data - Extension data (will be copied)
...
int SSL_set_hello_extension(SSL *s, int ext_type, void *ext_data, int ext_len)
{
    ...
    /* If ext_data is NULL, just clear the extension */
    if (ext_data == NULL || ext_len <= 0)
        return 1;

    /* Allocate and copy extension data */
    s->palm_ext.hello_ext = OPENSSL_malloc(sizeof(TLS_EXTENSION) + ext_len);
    if (s->palm_ext.hello_ext == NULL)
        return 0;

    s->palm_ext.hello_ext->type = ext_type;
    s->palm_ext.hello_ext->length = ext_len;
    s->palm_ext.hello_ext->data = (unsigned char *)(s->palm_ext.hello_ext + 1);
    memcpy(s->palm_ext.hello_ext->data, ext_data, ext_len);

    return 1;
}

/*
 * SSL_set_hello_extension_cb - Set callback for received TLS extensions
 *
 * This is a Palm/webOS compatibility function. It sets a callback that
 * will be invoked when a TLS extension is received in ServerHello.
 *
 * Note: In OpenSSL 1.1.1, custom extension parsing is done via callbacks
 * registered at the SSL_CTX level. This function stores the callback
 * for compatibility, but applications should use SSL_CTX_add_custom_ext()
 * for full functionality.
 *
 * Parameters:
 *   s   - SSL connection
 *   cb  - Callback function (returns 0 on success, alert code on error)
 *   arg - User argument passed to callback
 *
 * Returns 1 on success, 0 on failure
 */
int SSL_set_hello_extension_cb(SSL *s, tls_extension_cb_fn cb, void *arg)
{
    if (s == NULL)
        return 0;

    s->palm_ext.ext_cb = cb;
    s->palm_ext.ext_cb_arg = arg;
