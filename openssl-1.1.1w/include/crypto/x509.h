...
struct x509_store_ctx_st {      /* X509_STORE_CTX */
    X509_STORE *ctx;
    /*
     * WebKit ABI: 0.9.8 had an `int current_method;` here, so libWebKitLuna
     * inlines X509_STORE_CTX_get_current_cert as a read of *(ctx+8). 1.1.1
     * removed that field (pushing cert to offset 4); re-add the 4 bytes so the
     * device browser's sslVerificationCallback reads the real ->cert at offset 8.
     */
    int _wk_pad_current_method;
